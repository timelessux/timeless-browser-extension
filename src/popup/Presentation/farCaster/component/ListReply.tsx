...
const ListReply = ({ parentPost, setParentPost }: Props) => {
  const {
    getPostComments,
    comments,
    loadingComment,
    ...
    nextCursorComment,
    createComment
  } = useFarcasterViewModel()

  useEffect(() => {
    if (parentPost.id) getPostComments({ postId: parentPost.id, cursor: null, isFetchMore: false })
  }, [])
  ...
  const onClickComment = async (content: string, title: string) => {
    ...
    if (status === EStatusReact.DONE) {
      ...
      if (postClone.commentCount) postClone.commentCount = postClone.commentCount + 1
  ...
  return (
    <div className="list-social">
      ...
      >
        ...
        >
          {comments?.map((cmt) => {
            return <ReplyCard key={cmt.id} post={cmt} disabled={false} />
          })}
        </InfiniteScroll>
